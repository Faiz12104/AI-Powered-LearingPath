...
export const uploadDocument = async (req, res, next) => {
  try {
    ...
    // ✅ Background processing (PDF → chunks)
...
const processPDF = async (documentId, filePath) => {
  try {
    ...
    const text = await extractTextFromPDF(filePath); // ✅ FIX
...
export const getDocument = async (req, res, next) => {
  try {
    ...
    // Update access time without revalidating possibly legacy chunk data.
    await Document.updateOne(
      { _id: document._id },
      { $set: { lastAccessed: new Date() } }
    );

    // Attach counts to plain object
    const documentData = {
      ...document,
      flashcardCount,
      quizCount,
...
export const deleteDocument = async (req, res, next) => {
  try {
    ...
    if (document.filePath) {
      // Convert URL back to local file path
    ...
    // Delete related flashcards & quizzes (optional but recommended)
